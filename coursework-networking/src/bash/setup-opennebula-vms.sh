...
declare -A TEMPLATE
SWITCH_TEMPLATE_ID=2841
ATTACKER_TEMPLATE_ID=2865
TEMPLATE_ID_LIST=(2841 2865)
TEMPLATE["$SWITCH_TEMPLATE_ID,DESC"]="Switch machine template"
TEMPLATE["$ATTACKER_TEMPLATE_ID,DESC"]="Attacker machine template"
...
required_packages=("opennebula-tools" "ansible" "sshpass")
...
for VM_ID in "${VM_ID_LIST[@]}"; do
    ...
    echo -e "${GREEN}Successfully retrieved info about ${VM_DATA["$VM_ID,NAME"]}\n\
        ...
        [HTTP] Port 80: ${VM_DATA["$VM_ID,PORT_80"]}${CRESET}"

    if [ "${VM_DATA["$VM_ID,TEMPLATE_ID"]}" == "$SWITCH_TEMPLATE_ID" ]; then
    ...
    if [ "${VM_DATA["$VM_ID,TEMPLATE_ID"]}" == "$ATTACKER_TEMPLATE_ID" ]; then
