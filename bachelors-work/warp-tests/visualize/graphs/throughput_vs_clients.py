"""
Graph B1: Throughput vs Client Count
Line chart showing scalability with increasing concurrent clients.
"""
...
def plot_throughput_vs_clients():
    ...
    for variant in VARIANTS:
        ...
        # Plot PUT
        ax1.plot(client_counts, put_throughputs, marker=marker, color=color,
                 linestyle=linestyle, label=label, linewidth=2.5, markersize=7)

        # Plot GET
        ax2.plot(client_counts, get_throughputs, marker=marker, color=color,
                 linestyle=linestyle, label=label, linewidth=2.5, markersize=7)

    # Configure PUT plot
    ax1.set_title('PUT Throughput vs Client Count',
    ...
    # Configure GET plot
    ax2.set_title('GET Throughput vs Client Count',
                  fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    output_file = OUTPUT_DIR / 'graph_b_throughput_vs_clients.pdf'
